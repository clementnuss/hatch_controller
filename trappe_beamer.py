...
def main():
    ...
    from beamer.mqtt import mqtt_loop
    mqtt_thread = threading.Thread(
        target=mqtt_loop, name="MQTT-Thread")
    mqtt_thread.start()

    from beamer.beamer_state_machine import coverFSM
    logging.info("Starting cover state machine (Trappe beamer)")
    cover_thread = threading.Thread(
        target=coverFSM.control_loop, name="coverFSM-thread", daemon=True)
    cover_thread.start()

    logging.info("Starting 'trappe beamer' main Python")
